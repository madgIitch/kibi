...
public class CharacterEditorUI : MonoBehaviour
{
    ...
    public TMP_Text vHonesty, vEmotionality, vExtraversion, vAgreeableness, vConscientiousness, vOpenness, vHumor;
    ...
    void Start()
    {
        ...
        SetupSlider(sHumor,            v => { cfg.Humor            = v; UpdateLabel(vHumor, v); });

        if (nameInput)
            nameInput.onValueChanged.AddListener(t => cfg.displayName = t);

        // Inicializa UI con el config por defecto
        LoadUI(cfg);
        Apply();
    }
    ...
    void LoadUI(CharacterConfig c)
    {
        ...
        if (sHumor)             sHumor.value             = c.Humor;

        // Actualiza labels si existen
